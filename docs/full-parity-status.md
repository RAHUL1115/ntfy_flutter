...
`test/reference_goldens_test.dart` deterministically captures all nine supplied
...
| `flutter test --no-pub` | Pass, 136 tests; the opt-in real-server test is skipped in the default run |
...
| `flutter build apk --release` | Pass, 55.2 MB release APK |
